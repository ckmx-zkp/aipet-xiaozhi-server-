...
### 音乐与内容 MCP 自动交付（2026-09-08 用户授权）

音乐与内容工具源工程为 D:/xiaozhi_music_mcp，执行该仓 AGENTS.md 的自动交付约定。以后相关功能修改完成后，主动验证、构建、部署39.107.143.71受影响服务，再提交推送相关仓库并更新看板，不重复请求授权。保留无关改动和其他运行服务；验证失败修复或回滚。纯文档/规则改动提交推送即可。该授权不默认包含旧服务器部署、固件烧录或凭据变更。
